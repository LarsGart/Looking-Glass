...
Module.register('MMM-Face-Reco-DNN', {
  ...
  login_user: function(name) {
    ...
    // write the name of the user and time of login to a log file called userlogger.txt in directory \home\glass\userlogger.txt
    var fs = require('fs');
    var util = require('util');
    var log_file = fs.createWriteStream(__dirname + '/userlogger.txt', {flags : 'a'}); // if the file does not exist, it will be created. File wil be created in the same directory as this module
    var log_stdout = process.stdout;
    console.log = function(d) { //
      // name logged in at time
      log_file.write(util.format(d) + ' ' + name + ' logged in at ' + new Date());
      log_stdout.write(util.format(d) + ' ' + name + ' logged in at ' + new Date());
    };
    // log that the name was written to the file
